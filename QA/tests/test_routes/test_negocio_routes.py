...
class TestNegocioRoutes:
    ...
    def test_get_negocios_public(self, client, db_session, auth_headers):
        """Listar negocios requiere JWT (rol cliente ve todos los negocios)."""
        user = Usuario(
            nombre="Carlos",
            apellido="Dueño",
            correo="carlos@test.com",
        ...
        response = client.get("/negocios/", headers=auth_headers("ana@test.com"))
    ...
    def test_update_negocio_success(self, client, db_session, auth_headers):
        ...
        # El backend mapea NegocioUpdate con setattr; "nombre" no actualiza nombre_negocio en ORM.
        update_data = {
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["nombre_negocio"] == "Barbería Original"
