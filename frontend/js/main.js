...
function setVisible(show, hide = TODAS) {
  ...
  if (Array.isArray(show) && show.includes("registrar-negocio")) {
    setTimeout(() => {
      if (typeof window.inicializarMapaRegistroNegocio === "function") {
        window.inicializarMapaRegistroNegocio();
      }
    }, 350);
  }
...
function mostrarDashboardNegocio() {
  setVisible(["dashboard-negocio"]);
  cargarDashboardNegocioMetricas();
}
...
function mostrarRegistrarNegocio() {
  setVisible(["registrar-negocio"]);
  setTimeout(() => {
    if (typeof window.inicializarMapaRegistroNegocio === "function") {
      window.inicializarMapaRegistroNegocio();
    }
...
function inyectarEstilosModalSIGIE() {
  if (document.getElementById("sigi-modal-styles")) return;

  const style = document.createElement("style");
  style.id = "sigi-modal-styles";
  style.textContent = `
    .sigi-modal-overlay {
      position: fixed;
      inset: 0;
    ...
    .sigi-modal-card {
      ...
      background:
        ...
        linear-gradient(145deg, #12110d, #070707);
      color: #f8f4e8;
    ...
    .sigi-modal-overlay.show .sigi-modal-card {
    ...
    .sigi-modal-actions button {
      min-width: 130px;
    }

    .sigi-modal-card.danger {
      border-color: rgba(180, 38, 38, 0.58);
    }

    .sigi-modal-card.danger .sigi-modal-kicker {
      color: #ff7777;
    }
  `;
...
async function pedirConfirmacion({
  ...
  return new Promise((resolve) => {
    ...
    overlay.className = "sigi-modal-overlay";
    overlay.innerHTML = `
      <div class="sigi-modal-card ${danger ? "danger" : ""}" role="dialog" aria-modal="true">
        <div class="sigi-modal-kicker">${danger ? "Acción delicada" : "Confirmación"}</div>
        <h3>${escapeHtml(title)}</h3>
        <p>${escapeHtml(mensaje)}</p>
        <div class="sigi-modal-actions">
          ${hideCancel ? "" : `<button type="button" class="btn-secondary sigi-modal-cancel">${escapeHtml(cancelText || "Cancelar")}</button>`}
          <button type="button" class="${danger ? "btn-danger" : "btn-primary"} sigi-modal-confirm">${escapeHtml(confirmText || "Aceptar")}</button>
        </div>
      </div>
    `;
    ...
    requestAnimationFrame(() => {
      ...
      overlay.querySelector(".sigi-modal-confirm")?.focus();
    });

    overlay.querySelector(".sigi-modal-confirm")?.addEventListener("click", () => close(true));
    overlay.querySelector(".sigi-modal-cancel")?.addEventListener("click", () => close(false));

    overlay.addEventListener("click", (event) => {
      if (event.target === overlay && !hideCancel) close(false);
    });
  ...
  try {
    ...
    cont.innerHTML = horarios.map(h => `
      <article class="admin-item">
        <div><h4>${escapeHtml(nombreDiaSemana(h.dia_semana))}</h4><p>${escapeHtml(h.hora_inicio || "--:--")} - ${escapeHtml(h.hora_fin || "--:--")} · ${h.disponible === false ? "No disponible" : "Disponible"}</p></div>
...
let mapaRegistroNegocio = null;
let marcadorRegistroNegocio = null;

function setUbicacionRegistroNegocio(lat, lng) {
  const inputLat = document.getElementById("negLatitud");
  const inputLng = document.getElementById("negLongitud");
  const texto = document.getElementById("negUbicacionTexto");

  if (inputLat) inputLat.value = Number(lat).toFixed(6);
  if (inputLng) inputLng.value = Number(lng).toFixed(6);
  if (texto) texto.textContent = `Ubicación seleccionada: ${Number(lat).toFixed(6)}, ${Number(lng).toFixed(6)}`;
}

function inicializarMapaRegistroNegocio() {
  const contenedor = document.getElementById("mapaRegistroNegocio");
  if (!contenedor || typeof L === "undefined") return;

  const centroInicial = [4.7110, -74.0721]; // Bogotá por defecto

  if (!mapaRegistroNegocio) {
    mapaRegistroNegocio = L.map("mapaRegistroNegocio").setView(centroInicial, 12);

    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
      maxZoom: 19,
      attribution: "&copy; OpenStreetMap"
    }).addTo(mapaRegistroNegocio);

    mapaRegistroNegocio.on("click", (event) => {
      const { lat, lng } = event.latlng;

      if (!marcadorRegistroNegocio) {
        marcadorRegistroNegocio = L.marker([lat, lng], { draggable: true }).addTo(mapaRegistroNegocio);

        marcadorRegistroNegocio.on("dragend", () => {
          const pos = marcadorRegistroNegocio.getLatLng();
          setUbicacionRegistroNegocio(pos.lat, pos.lng);
        });
      } else {
        marcadorRegistroNegocio.setLatLng([lat, lng]);
      }

      setUbicacionRegistroNegocio(lat, lng);
    });
  }

  setTimeout(() => mapaRegistroNegocio.invalidateSize(), 250);
}

window.inicializarMapaRegistroNegocio = inicializarMapaRegistroNegocio;
  ...
  document.getElementById("negocioForm")?.addEventListener("submit", async (e) => {
    ...
    const latitud = Number(document.getElementById("negLatitud")?.value);
    const longitud = Number(document.getElementById("negLongitud")?.value);

    if (!Number.isFinite(latitud) || !Number.isFinite(longitud)) {
      mostrarMensaje("negocioMsg", "Debes seleccionar la ubicación del negocio en el mapa.");
      return;
    }
    ...
    try {
      ...
      if (res.status === 201 || res.ok) {
        ...
        document.getElementById("negLatitud").value = "";
        document.getElementById("negLongitud").value = "";
        document.getElementById("negUbicacionTexto").textContent = "No has seleccionado ubicación.";
        if (marcadorRegistroNegocio) {
          mapaRegistroNegocio.removeLayer(marcadorRegistroNegocio);
          marcadorRegistroNegocio = null;
        }
...
function destruirChartNegocio(id) {
  if (chartsDashboardNegocio[id]) {
    chartsDashboardNegocio[id].destroy();
    delete chartsDashboardNegocio[id];
  }
}

function crearChartNegocio(idCanvas, tipo, labels, data, label) {
  console.log("PINTANDO_CHART_NEGOCIO", idCanvas, labels, data);
  const canvas = document.getElementById(idCanvas);
  if (!canvas || typeof Chart === "undefined") return;

  destruirChartNegocio(idCanvas);

  chartsDashboardNegocio[idCanvas] = new Chart(canvas, {
    type: tipo,
    data: {
      labels,
      datasets: [{
...
function pintarListaDashboardNegocio(id, items, renderItem, emptyText) {
  const cont = document.getElementById(id);
  if (!cont) return;

  if (!Array.isArray(items) || items.length === 0) {
    cont.innerHTML = `<div class="empty-state">${escapeHtml(emptyText)}</div>`;
    return;
  }

  cont.innerHTML = items.map(renderItem).join("");
}

async function cargarDashboardNegocioMetricas(force = false) {
  const section = document.getElementById("anaNegocioSection");
  if (!section) return;

  try {
    const negocio = await obtenerMiNegocio(force);
    const idNegocio = obtenerIdNegocio(negocio);
    ...
    const data = await apiRequest(`/dashboard/negocio/${idNegocio}/metricas`);
    console.log("METRICAS_NEGOCIO_DATA", data);
    ...
    setText("anaMetTotalCitas", resumen.total_citas ?? 0);
    setText("anaMetIngresosMes", formatearMonedaCOP(resumen.ingresos_mes));
    setText("anaMetPedidosMes", resumen.pedidos_mes ?? 0);
    setText("anaMetStockBajo", resumen.productos_stock_bajo ?? 0);

    crearChartNegocio(
      "chartCitasMes",
      "bar",
      (data.citas_por_mes || []).map(x => x.mes),
      (data.citas_por_mes || []).map(x => Number(x.total || 0)),
      "Citas"
    );
    ...
    crearChartNegocio(
      "chartVentasMes",
      "line",
      (data.ventas_por_mes || []).map(x => x.mes),
      (data.ventas_por_mes || []).map(x => Number(x.total_ventas || 0)),
      "Ventas"
    );
    ...
    crearChartNegocio(
      "chartEstadosCitas",
      "doughnut",
      (data.estados_citas || []).map(x => x.estado),
      (data.estados_citas || []).map(x => Number(x.total || 0)),
      "Estados"
    );
    ...
    crearChartNegocio(
      "chartServUsados",
      "bar",
      (data.servicios_mas_agendados || []).map(x => x.nombre),
      (data.servicios_mas_agendados || []).map(x => Number(x.total_agendado || 0)),
      "Agendamientos"
    );
    ...
    crearChartNegocio(
      ...
      "bar",
      (data.productos_mas_vendidos || []).map(x => x.nombre),
      (data.productos_mas_vendidos || []).map(x => Number(x.unidades_vendidas || 0)),
      "Unidades vendidas"
    );

    pintarListaDashboardNegocio(
      "anaTopEmpleados",
      data.top_empleados || [],
      (e) => `
        <div class="admin-item compact">
          <div>
            <h4>${escapeHtml(e.empleado || "Empleado")}</h4>
            <p>${escapeHtml(e.especialidad || "Sin especialidad")} · ${Number(e.citas_exitosas || 0)} citas exitosas · ${formatearMonedaCOP(e.ingresos_generados)}</p>
          </div>
        </div>
      `,
      "Aún no hay citas suficientes para calcular top empleados."
    );
    ...
    pintarListaDashboardNegocio(
      ...
      (p) => `
        <div class="admin-item compact">
          <div>
            <h4>${escapeHtml(p.nombre || "Producto")}</h4>
            <p>Stock: ${Number(p.stock || 0)} · Precio: ${formatearMonedaCOP(p.precio)}</p>
          </div>
        </div>
      `,
    ...
    section.style.display = "block";
    section.innerHTML = `<div class="empty-state">No se pudieron cargar las métricas del negocio: ${escapeHtml(obtenerMensajeError(error))}</div>`;
  }
}
...
async function cargarMisCalificaciones() {
  ...
  try {
    const [calificaciones, pendientes, ranking] = await Promise.all([
    ...
    const rankingNegocios = Array.isArray(ranking) ? ranking : [];
    ...
    const rankingHtml = rankingNegocios.length
      ? rankingNegocios.map(r => {
          const promedio = Number(r.promedio || 0).toFixed(1);
          const total = Number(r.total_calificaciones || 0);

          return `<article class="admin-item">
            <div>
              <h4>#${escapeHtml(r.posicion)} · ${escapeHtml(r.nombre_negocio || "Negocio")}</h4>
              <p>
                ${escapeHtml(estrellasPromedio(r.promedio))}
                · Promedio: ${escapeHtml(promedio)}
                · ${escapeHtml(total)} calificación${total === 1 ? "" : "es"}
              </p>
              <span class="status-pill done">Ranking</span>
            </div>
          </article>`;
        }).join("")
      : `<div class="empty-state">Todavía no hay ranking de negocios.</div>`;
...
async function eliminarUsuarioAdmin(idUsuario) {
  try {
    ...
    // Refresca la tabla inmediatamente después de eliminar.
    await cargarUsuariosAdmin();
